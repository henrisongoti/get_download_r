...
lnk.url<-"https://data.chc.ucsb.edu/products/CHIRP/daily"
...
dir_save_nc <-"/data_cdt/SATELLITE_DATA/model_data/chirp/nc"
if(!dir.exists(dir_save_nc)){dir.create(dir_save_nc,recursive=TRUE)}
dir_save_tiff_globe <-"/data_cdt/SATELLITE_DATA/model_data/chirp/tiff/Afrique"
if(!dir.exists(dir_save_tiff_globe)){dir.create(dir_save_tiff_globe,recursive=TRUE)}
dir_save_tiff_cilss <-"/data_cdt/SATELLITE_DATA/model_data/chirp/tiff/Cilss"
...
all_link <-paste(lnk.url,dd.tmp1[, 1],
                 paste("chirp.", dd.tmp1[, 1], ".", dd.tmp1[, 2], ".", dd.tmp1[, 3], ".tif.gz",sep = ""),sep = "/")
...
to.down<- all_link[!(unlist(str_extract_all(unlist(all_link),
                                            paste("chirp.", dd.tmp1[, 1], ".", dd.tmp1[, 2], ".", dd.tmp1[, 3], ".tif",sep = "")))%in%already.down)]
...
for(i in seq(to.down)){ 
  ...
  if(!inherits(t1, "try-error")){
    ...
    missval <- -99
    longname <- "Precipitation Estimation chirp"
