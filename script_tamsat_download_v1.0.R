...
lnk.url<-"https://gws-access.jasmin.ac.uk/public/tamsat/rfe/data/v3.1/daily"
...
#todays <- today(tzone = "")-5
todays <- today(tzone = "")
firstday <- ceiling_date(todays, 'month') - days(1) #dernier jour du mois en cours
lastday <-floor_date(todays, 'month') - days(1)   #dernier jour du mois précédent
todays <- today(tzone = "")
jour_mois <- day(todays)
if (jour_mois <= 5 ) {
	todays = floor_date(todays, 'month') - days(1)

}

if (jour_mois >= 6 &  jour_mois <= 10 ) {
	todays = floor_date(todays, 'month') - days(1) + days(5)

}

if (jour_mois >= 11 &  jour_mois <= 15 ) {
	todays = floor_date(todays, 'month') - days(1) + days(10)

}

if (jour_mois >= 16 &  jour_mois <= 20 ) {
	todays = floor_date(todays, 'month') - days(1) + days(15)

}

if (jour_mois >= 21 &  jour_mois <= 25 ) {
	todays = floor_date(todays, 'month') - days(1) + days(20)

}

if (jour_mois >= 26 ) {
	todays = floor_date(todays, 'month') - days(1) + days(25)
...
print(todays)
print(jour_mois)
#todays <- ymd("2023-04-25")
print(todays)
#start_date <- ymd("2022-01-01")
#todays <- ymd("2022-03-03")
...
url=to.down[[i]]
myfile=paste(dir_save_nc,basename(to.down[[i]]),sep = "/")

print(url)
print(myfile)
#curl_download(url,myfile)
...
t1=try(download.file(to.down[[i]],destfile = paste(dir_save_nc,basename(to.down[[i]]),sep = "/"),handle = h,mode="wb",method="wget",extra = "--no-check-certificate"),silent = T)
#t1=try(download.file(to.down[[i]],destfile = paste(dir_save_nc,basename(to.down[[i]]),sep = "/"),handle = h,mode="wb",method="wget",extra = getOption("--random wait", "-r", "-p","--no-check-certificate")),silent = T)
print(to.down[[i]])
#print(t1)
#t1=try(curl::curl_download(to.down[[i]],destfile = paste(dir_save_nc,basename(to.down[[i]]),sep = "/"),handle = h),silent = T)
...
dossier <- "/data_cdt/SATELLITE_DATA/model_data/tamsat3.1/nc/"

# Liste des fichiers dans le dossier
fichiers <- list.files(dossier, full.names = TRUE)

# Parcourir les fichiers et supprimer ceux de taille 0
for (fichier in fichiers) {
  taille <- file.info(fichier)$size
  if (taille == 0) {
    file.remove(fichier)
    cat("Fichier supprimé:", fichier, "\n")
  }
}
